...
<body>
  <h1>
    <a href="index.html">航迹云</a>
  </h1>
  <p>
    盛夏，这个季节肆意的支配能够延伸到骄阳灼目的光线所能到达的每一个角落，即便是凛冽的北风也无法逃脱；深秋，这个季节带来了慷慨的惠赠，同时也带来了肃杀的诗意；寒冬，这个季节重新引领着呼啸的北风驱赶一切，却又温柔地令雪花飞舞，降下寂静。季节更迭，只有航迹云独自在空中映出了一成不变的景色，偶尔发出低沉的私语。这是一个绽放于盛夏，历经秋日的留恋，最终在冬日落幕的，诗人与航迹云的故事。
  </p>
...
repeat
    ...
    if index[pre_ch_code] then
        pre_in_a = a_template:gsub("%[ch%-code%]", ("%03d"):format(pre_ch_code))
                       :gsub("%[ch%-title%]", index[pre_ch_code])
    ...
    -- write html
    ch_in_p = ch:gsub("\n", "</p>\n    <p>"):gsub("<p></p>", "<p>&nbsp;</p>")
    html = ch_template:gsub("%[ch%-code%]", ("%03d"):format(ch_code)):gsub(
               "%[ch%-title%]", ch_title)
    html = html:gsub("%[pre%-item%]", pre_in_a)
    html = html:gsub("%[post%-item%]", post_in_a)
    html = html:gsub("%[txt%]", "<p>" .. ch_in_p .. "</p>")
    io.open("../story/" .. ("%03d"):format(ch_code) .. ".html", "w"):write(html)
        :close()
    ...
    -- make a
    a = a_template:gsub("%[ch%-code%]", ("%03d"):format(ch_code)):gsub(
            "%[ch%-title%]", ch_title)
    ...
    if pre_ch_code > 0 then
        ...
        file = io.open('../story/' .. ("%03d"):format(pre_ch_code) .. '.html',
                       'r')
        pre_html = file:read('a')
        file:close()
        pre_html = pre_html:gsub('未完待续……', a)
        io.open('../story/' .. ("%03d"):format(pre_ch_code) .. '.html', 'w'):write(
            pre_html):close()
    end
...
-- rewrite index file content
local content_item = a_template:gsub("%.%.%/", "")
file = io.open("../content.html", "w")
file:write(content_before)
for i, v in ipairs(index) do
    file:write([[
      <li>
        <p>]],
               a_template:gsub("%.%.%/", ""):gsub("%[ch%-code%]",
                                                  ("%03d"):format(i)):gsub(
                   "%[ch%-title%]", v), [[</p>
      </li>
]])
